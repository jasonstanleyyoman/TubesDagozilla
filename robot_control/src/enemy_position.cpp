...
void enemyPositionCallback(const gazebo_msgs::ModelStates::ConstPtr& message_holder){
	robot_control::ModelState sent_message;
	
	int size = 7;
	for(int i = 0; i < size;i++){
		if(message_holder->name[i] == "robot2"){
...
int main(int argc, char *argv[]){
	...
	ball_position_publisher = nh.advertise<robot_control::ModelState>("/enemy_position",1,true);
	ball_position_subscriber = nh.subscribe("/gazebo/model_states",1,enemyPositionCallback);
